...
public class PriorityScheduling implements Processable{

    List<Job> jobs;

    public PriorityScheduling(List<Job> jobs) {
        this.jobs = jobs.stream().sorted(
                Comparator.comparingInt((Job j) -> j.priority)
                        .thenComparingInt(j->j.arrivalTime)
        ).toList();
    }
    ...
    private boolean isJobReadyToBeProcessed(Job ongoingJob, int i, int currentTime) {
        Job candidate = jobs.get(i);

        return candidate.arrivalTime <= currentTime
            && candidate.remainingTime > 0
            && (
                    ongoingJob == null
                            || candidate.priority < ongoingJob.priority
                            || isToTieBrakingResolver(candidate, ongoingJob)
                );

    }

    private boolean isToTieBrakingResolver(Job candidate, Job ongoingJob) {
        return candidate.priority == ongoingJob.priority
                && candidate.arrivalTime < ongoingJob.arrivalTime;
    }
